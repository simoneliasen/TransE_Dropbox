...
def WNexp(state, channel):
    ...
    n_batches = int(math.ceil(float(trainl.shape[1]) / float(state.batch_size)))
    ...
    for epoch_count in xrange(1, state.totepochs + 1):
        ...
        for i in range(n_batches):
            to_idx = min((i + 1) * state.batch_size, trainl.shape[1])

            tmpl = trainl[:, i * state.batch_size:to_idx]
            tmpr = trainr[:, i * state.batch_size:to_idx]
            tmpo = traino[:, i * state.batch_size:to_idx]
            tmpnl = trainln[:, i * state.batch_size:to_idx]
            tmpnr = trainrn[:, i * state.batch_size:to_idx]
            # training iteration
            outtmp = trainfunc(state.lremb, state.lrparam / float(state.batch_size),
                    tmpl, tmpr, tmpo, tmpnl, tmpnr)
            out += [outtmp[0] / float(state.batch_size)]
            outb += [outtmp[1]]
