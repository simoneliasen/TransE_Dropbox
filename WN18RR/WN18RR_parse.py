...
for datatyp in ['train', 'valid', 'test']:
    f = open(datapath + '%s.txt' % datatyp, 'r')
    dat = f.readlines()
    f.close()
    for i in dat:
...
f = open('../data/WN18RR_synset2idx.pkl', 'w')
g = open('../data/WN18RR_idx2synset.pkl', 'w')
...
f = open("../raw_data/wn/wordnet-mlj12-definitions.txt", 'r')
dat = f.readlines()
f.close()
...
f = open('../data/WN18RR_synset2def.pkl', 'w')
g = open('../data/WN18RR_synset2concept.pkl', 'w')
...
for datatyp in ['train', 'valid', 'test']:
    ...
    if 'data' not in os.listdir('../'):
        os.mkdir('../data')
    f = open('../data/WN18RR-%s-lhs.pkl' % datatyp, 'w')
    g = open('../data/WN18RR-%s-rhs.pkl' % datatyp, 'w')
    h = open('../data/WN18RR-%s-rel.pkl' % datatyp, 'w')
